...
### Compatibility with the `ok-cluster` consumer

The renderer is supplied by this repository, but the active profile and
lifecycle are owned by `openkubes/ok-cluster`. Its Makefile loads
`research/kagent-standalone` from `OPENKUBES_DIR`; it does not vendor these
files or select a Git revision automatically. A neighboring checkout at an
incompatible revision is therefore a real failure mode.

For every change to the allow-list or schema, run the coupled consumer test:

```bash
make -C <ok-cluster>/ok-kagent/kagent access-test \
  OPENKUBES_DIR=<this-openkubes-checkout>
```

The test must exercise the consumer's real `access-config.yaml`, and the
evidence output must record both repository SHAs. Do not merge a consumer
widening before the matching renderer is available on the target Openkubes
branch. This ordering error happened with `ok-cluster#27` and the closed,
unmerged `openkubes#50`; documenting it here is part of preventing a repeat.
...
Adding another resource kind still requires extending `WRITABLE_RESOURCES` and
its tests. Sensitive and cluster-scoped kinds in `FORBIDDEN_RESOURCES` are not
configurable.
...
- **Tool names** are config, not code. They must exist in the installed tool
  ...
  invent names.
- **More resource kinds** are added centrally to `WRITABLE_RESOURCES`; profiles
  can immediately select any subset of that supported list. Anything in

...
metadata:
  ...
  namespace: kagent
spec:
  # Upstream CRD shape only. The lab renderer grants only resources selected in
  # access-config.yaml — see §7.1. Keep the description inside that exact set.
  description: Inspects workloads and performs approval-gated scoped repairs.
  type: Declarative
  declarative:

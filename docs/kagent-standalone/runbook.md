...
The profile may select any non-empty subset of the ten intended OK-129 resource
kinds:
...
tool name in the ungated `read.tools` reference; unsupported resource kinds; and
every namespace target other than `kagent-lab`.
...
Exercise reversible, isolated objects only. A ConfigMap is the baseline flow;
repeat the same Approve/Reject/verify sequence with a safe fixture for every
additional resource selected in the active profile:
...
The recorded live drill used ConfigMaps in `kagent-lab`:
...
> **Delivered permission is not live evidence.** Pods, Services, Deployments,
> StatefulSets, DaemonSets, ReplicaSets, Jobs, CronJobs and Ingresses are valid
> selections and receive the declared Role verbs, but their Approve/Reject
> behaviour has not yet been recorded. The renderer's tests prove the *RBAC
> shape* — never the agent's behaviour with a rollout it can break. Ungated and
> cluster-wide writes remain refused.
...
- [ ] One namespace-scoped ConfigMap write flow passes Approve, Reject, and
      `ask_user`; every additional configured resource has either a recorded
      equivalent drill or is explicitly marked as permission-only evidence.

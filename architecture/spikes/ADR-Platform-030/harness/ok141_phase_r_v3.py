#!/usr/bin/env python3
"""Offline verifier for the additive OK-141 Phase-R v3 execution fixture."""
...
V2 = _module("ok141_phase_r_v2_for_v3", "ok141_phase_r_v2.py")
PLATFORM = _module("ok141_platform_for_v3", "ok141_platform_amendment.py")
V1 = V2.V1
FORMAT = "ok141-execution-fixture/v3"
VERSION = "phase-r-v3"
...
def _expect(actual: Any, expected: Any, claim: str) -> None:
    if actual != expected:
        raise V1.HarnessError(f"Phase-R v3 {claim} mismatch")
...
def validate(document: dict[str, Any], root: Path) -> str:
    ...
    _expect(supersedes.get("fixtureVersion"), "phase-r-v2", "superseded version")
    _expect(supersedes.get("fixtureDigest"), "sha256:b27bb7c8e959e2c1028fcc0822755caa795ce21432344a64a62474abeb7f9f2b", "superseded digest")
    ...
    p_revision = PLATFORM.validate_platform_amendment(profile, apps, provider_values)
    ...
    _expect(V1.sha256_bytes(HARNESS_DIR.joinpath("ok141_platform_amendment.py").read_bytes()), tools["platformAmendmentToolDigest"], "Platform tool digest")
    _expect(V1.sha256_bytes(Path(__file__).read_bytes()), tools["phaseRV3ToolDigest"], "Phase-R v3 tool digest")

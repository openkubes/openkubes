#!/usr/bin/env python3
"""Generate the immutable, non-authorizing OK-141 fresh-run v7 package."""
...
RUNNER_SOURCE_SHA = "4a9a650c4270b2a572e7de4a7d3f3b45b2837330"
RUNNER_RECEIPT_FORMAT = "ok147-runner-publication-receipt/v1"
WORKLOAD_API_ADDRESS = "192.168.100.213"
WORKLOAD_API_CIDR = WORKLOAD_API_ADDRESS + "/32"
EVIDENCE_COLLECTOR_ADDRESS = "192.168.100.214"
EVIDENCE_COLLECTOR_CIDR = EVIDENCE_COLLECTOR_ADDRESS + "/32"
...
def build_enablement() -> tuple[dict, str, str]:
    ...
    annotations.update({
        "openkubes.io/candidate-status": "fresh-run-v7-no-go",
...
def build_provider_access_policy() -> dict:
    return {
        "format": "ok147-provider-access-policy/v1",
...
def bind_workload_api_address(documents: list[dict]) -> None:
    kubevirt_cluster = next(item for item in documents if item.get("kind") == "KubevirtCluster")
    service = kubevirt_cluster["spec"]["controlPlaneServiceTemplate"]
    service["metadata"] = {
        "annotations": {"metallb.io/loadBalancerIPs": WORKLOAD_API_ADDRESS},
    }
...
def build_plan(inputs: dict[str, list[tuple[str, str]]]) -> dict:
    ...
    for index, (stage_id, kind, authority, operation) in enumerate(rules):
        stage = {
            ...
            "inputs": [
                {"name": name, "digest": input_digest}
                for name, input_digest in inputs[stage_id]
            ],
        }
        if operation:
...
def main() -> None:
    ...
    lifecycle_documents = load_documents(projection_dir / "ok-mgmt-lifecycle.yaml")
    ...
    provider_access_policy = build_provider_access_policy()
    write_json(ARTIFACTS / "provider-access-policy.json", provider_access_policy)
    ...
    inputs = {
        ...
        "cluster-lifecycle": [
            ("projection.cluster-lifecycle", digest_bytes(lifecycle)),
            ("stage.provider-access", raw_digest("provider-access-policy.json")),
        ],
    ...
    manifest = {
        "format": "ok141-fresh-run-package/v7",
        "authorizationState": "NO-GO",
        ...
        "artifacts": {name: digest_bytes(path.read_bytes()) for name, path in sorted({
            **{name: ARTIFACTS / name for name in json_artifacts},
            ...
            "cluster-lifecycle.yaml": ARTIFACTS / "cluster-lifecycle.yaml",
            "provider-access-policy.json": ARTIFACTS / "provider-access-policy.json",
        }.items())},
        "semanticDigests": {
        ...
        "runtimeNetwork": {
            ...
            "workloadAPI": {"address": WORKLOAD_API_ADDRESS, "cidr": WORKLOAD_API_CIDR, "port": 6443},
            "evidenceCollector": {"address": EVIDENCE_COLLECTOR_ADDRESS, "cidr": EVIDENCE_COLLECTOR_CIDR, "port": 8443},

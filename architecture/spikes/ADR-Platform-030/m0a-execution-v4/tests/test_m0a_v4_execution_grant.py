...
SPEC = importlib.util.spec_from_file_location("controlled_m0a_execution_v4_grant", ROOT / "controlled_m0a_execution_v4.py")
MODULE = importlib.util.module_from_spec(SPEC)
assert SPEC.loader
SPEC.loader.exec_module(MODULE)

CANDIDATE = ROOT / "m0a-execution-candidate-v4.yaml"
GRANT = ROOT / "m0a-combined-grant-v4.yaml"
GRANT_DIGEST = ROOT / "m0a-combined-grant-v4.sha256"
...
def test_exact_grant_and_digest_verify_in_window() -> None:
    ...
    result = MODULE.verify_grant(CANDIDATE, GRANT, datetime(2026, 8, 12, 19, 0, tzinfo=timezone.utc))
    assert result["maximumRuns"] == 1
    assert result["candidateDigest"] == MODULE.sha(CANDIDATE)
    assert result["evidenceOutputPath"] == "/private/tmp/ok141-m0a-v4-execution-evidence-20260812.json"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (("spec", "maximumRuns"), 2),
        (("spec", "evidenceOutputPath"), "/Users/arash/wrong.json"),
...
def test_grant_tampering_fails_closed(tmp_path: Path, path: tuple[str, ...], value) -> None:
    ...
    with pytest.raises(MODULE.ExecutionError):
        MODULE.verify_grant(CANDIDATE, write_grant(tmp_path, document), datetime(2026, 8, 12, 19, 0, tzinfo=timezone.utc))


def test_grant_is_not_valid_before_or_after_window() -> None:
    with pytest.raises(MODULE.ExecutionError, match="outside"):
        MODULE.verify_grant(CANDIDATE, GRANT, datetime(2026, 8, 12, 17, 59, tzinfo=timezone.utc))
    with pytest.raises(MODULE.ExecutionError, match="outside"):
        MODULE.verify_grant(CANDIDATE, GRANT, datetime(2026, 8, 12, 21, 1, tzinfo=timezone.utc))
...
def test_three_grant_ids_are_exact_and_distinct() -> None:
    ...
    assert [spec[key]["grantID"] for key in ("credentialGrant", "admissionGrant", "installationGrant")] == [
        "ok141-m0a-c1-v4-20260812-01",
        "ok141-m0a-a1-v4-20260812-01",
        "ok141-m0a-i-v4-20260812-01",
    ]

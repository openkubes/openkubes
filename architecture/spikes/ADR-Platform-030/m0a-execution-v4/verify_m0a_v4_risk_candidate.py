#!/usr/bin/env python3
"""Verify that the M0a v4 risk candidate remains exact and non-authorizing."""
...
def verify(path: Path) -> str:
    ...
    expect(spec["version"], "ok141-m0a-v4-risk-acceptance/v1", "version")
    ...
    resolve(path, spec["references"]["v3RuntimeEvidence"])
    if sha(security) not in spec["acceptanceText"]:

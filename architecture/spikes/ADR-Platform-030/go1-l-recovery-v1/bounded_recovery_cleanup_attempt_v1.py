#!/usr/bin/env python3
"""Cleanup executor bound to a reusable immutable R0 attempt."""
...
MODULE_SPEC = importlib.util.spec_from_file_location(
    "bounded_recovery_cleanup_v2_for_attempt", HERE / "bounded_recovery_cleanup_v2.py"
)
...
CANDIDATE = HERE / "recovery-cleanup-candidate-r0-v4-20260814-01.yaml"
TOOL = BASE.TOOL
...
def validate_candidate(candidate_path: Path = CANDIDATE) -> dict[str, Any]:
    ...
    BASE.expect(materializer["path"], "materialize_recovery_binding_attempt_v1.py", "materializer path")
    for claim in ("sourceObservationCandidateDigest", "attemptID", "bindingName"):
        if not materializer.get(claim):
            raise CleanupError(f"candidate lacks {claim}")
    return candidate
...
def validate_binding(
    ...
    configured = candidate["spec"]["runtimeBindingMaterializer"]
    BASE.expect(binding["metadata"]["name"], configured["bindingName"], "binding name")
    spec = binding["spec"]
    BASE.expect(spec.get("bindingVersion"), BINDING_VERSION, "binding version")
    BASE.expect(spec.get("attemptID"), configured["attemptID"], "binding attempt")
    BASE.expect(
        spec.get("sourceObservationCandidateDigest"),
        configured["sourceObservationCandidateDigest"],
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "verify":
            ...
            result = execute_once(
                candidate_path, args.binding.resolve(), args.grant.resolve(), args.stage,

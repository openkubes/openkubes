#!/usr/bin/env python3
"""Fail-closed offline verification for the OK-141 fresh-run v8 package."""
...
import generate_fresh_run_v8 as generator
...
def verify(root: Path) -> str:
    ...
    expect(manifest["format"], "ok141-fresh-run-package/v8", "package format")
    ...
    expect(len(target_access), 11, "target-access membership")
    expect(
        [(item["kind"], item["metadata"].get("namespace", ""), item["metadata"]["name"]) for item in target_access[-3:]],
        [
            ("ServiceAccount", "ok-observability", "ok147-observability-autonomy"),
            ("Role", "ok-observability", "ok147-observability-autonomy"),
            ("RoleBinding", "ok-observability", "ok147-observability-autonomy"),
        ],
        "autonomy target-access membership",
    )

#!/usr/bin/env python3
"""Bounded exact-GET observer after the OK-141 Platform network amendment."""
...
def get(uri: str) -> dict:
    result = subprocess.run([str(CLIENT), "--kubeconfig", str(KUBECONFIG), "get", "--raw", uri], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False)
    if result.returncode != 0:
        raise ObserverError("exact Application GET failed")
    return json.loads(result.stdout)


def summary(value: dict, expected: dict) -> dict:
    ...
    condition_types = sorted({item.get("type", "UNKNOWN") for item in conditions})
    identity_current = (
    ...
    ready = identity_current and fresh and sync.get("status") == "Synced" and sync.get("revision") == expected["sourceRevision"] and health.get("status") == "Healthy" and not conditions
    return {
...
def main() -> int:
    ...
    try:
        ...
        if sha(predecessor) != spec["predecessor"]["digest"] or json.loads(predecessor.read_text()).get("state") != "PASS-AMENDED":
        ...
        for iteration in range(1, spec["polling"]["maxIterations"] + 1):
            ...
            for name in spec["applications"]:
                ...
                final.append(summary(get(uri), spec["expected"]))
        ...
        evidence = {
            ...
            "kind": "GO1PostPlatformNetworkAmendmentEvidence",
        ...
        with os.fdopen(descriptor, "w") as stream:
            ...
            stream.write("\n")
        print(json.dumps({"allReady": evidence["allReady"], "readyCount": evidence["readyCount"], "iterations": evidence["iterations"], "evidenceDigest": sha(output)}, sort_keys=True))
        return 0

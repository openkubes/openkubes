#!/usr/bin/env python3
"""Review exact effective Argo target permissions without retaining credentials."""
from __future__ import annotations
import base64,hashlib,json,os,subprocess
from pathlib import Path
import yaml

HERE=Path(__file__).resolve().parent;CANDIDATE=HERE/"effective-rbac-review-v1.yaml"
...
def main():
 ...
 if sha(pred)!=spec["predecessor"]["digest"]:raise RuntimeError("predecessor mismatch")
 if set(spec["exactResourceNames"])!=NAMES or set(spec["exactVerbs"])!={"bind","escalate"}:raise RuntimeError("scope mismatch")
 ...
 result={"apiVersion":"evidence.openkubes.io/v1alpha1","kind":"GO1EffectiveRBACReviewEvidence","candidateDigest":sha(CANDIDATE),"predecessorDigest":spec["predecessor"]["digest"],"reviews":[],"exactReviewCount":0,"registrationSecretReadPerformed":False,"temporaryKubeconfigRemoved":False,"credentialPayloadRetained":False,"endpointRetained":False,"rawResponsesRetained":False,"persistentResourceCreated":False,"rbacChanged":False,"retryPerformed":False,"cleanupPerformed":False,"failureInjectionPerformed":False}
 try:
  secret=run(SHARED,"get",spec["registrationSecretURI"]);result["registrationSecretReadPerformed"]=True
  server=decode(secret,"server");config=json.loads(decode(secret,"config"));token=config["bearerToken"];ca=config["tlsClientConfig"]["caData"]
  ...
  token="";ca="";server="";config={};secret={};kube={}
  uri="/apis/authorization.k8s.io/v1/selfsubjectaccessreviews"
  for name in sorted(NAMES):
   for verb in sorted(spec["exactVerbs"]):
    ...
    response=run(ephemeral,"create",uri,json.dumps(body,separators=(",",":")).encode());status=response.get("status",{})
    result["reviews"].append({"name":name,"verb":verb,"allowed":status.get("allowed") is True,"denied":status.get("denied") is True})
  result["exactReviewCount"]=len(result["reviews"]);result["allAllowed"]=all(x["allowed"] for x in result["reviews"]);result["state"]="PASS-EFFECTIVE-RBAC-REVIEW"
 finally:
 ...
 print(json.dumps({"state":result.get("state"),"allAllowed":result.get("allAllowed"),"reviews":result.get("reviews"),"evidenceDigest":sha(output)},sort_keys=True));return 0

...
from verify_delete_d0_live_grant_candidate_v1 import (
    CandidateError,
    canonical_digest,
    file_digest,
    verify_candidate,
...
CANDIDATE = HERE / "delete-d0-live-grant-candidate-v1.yaml"


class D0LiveGrantCandidateTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_candidate_has_no_live_window(self) -> None:
        def mutate(value):
            value["spec"]["requiredGrant"]["notBefore"] = "2026-08-20T14:00:00Z"
        with self.assertRaisesRegex(CandidateError, "live grant values"):
            verify_candidate(self.changed_candidate(mutate))

    def test_delete_authority_cannot_be_removed_from_required_false(self) -> None:
        def mutate(value):
    ...
    def test_existing_d0_candidate_is_digest_bound(self) -> None:
        verified = verify_candidate(CANDIDATE)
    ...
    def test_publication_candidate_is_bound_and_not_authorizing(self) -> None:
        publication = yaml.safe_load((HERE / "delete-d0-live-grant-publication-candidate-v1.yaml").read_text())
        ...
        self.assertEqual(8, spec["bindings"]["offlineTestsPassed"])

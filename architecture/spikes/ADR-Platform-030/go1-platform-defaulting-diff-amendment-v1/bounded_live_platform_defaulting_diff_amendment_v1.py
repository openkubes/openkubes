#!/usr/bin/env python3
"""Apply the exact OK-141 v8 defaulting-diff amendment once."""
...
def execute(candidate_path: Path) -> dict[str, Any]:
    ...
    if digest_file(client) != spec["clientDigest"]:
        raise LiveDefaultingError("client digest mismatch")
    for path in (mgmt, shared):
        if path.is_symlink() or not path.is_file() or stat.S_IMODE(path.stat().st_mode) != 0o600:
            raise LiveDefaultingError("unsafe kubeconfig")
    if output.exists() or output.is_symlink():
        raise LiveDefaultingError("exclusive output exists")
    ...
    for item in lifecycle:
        ...
        if (
            ...
            raise LiveDefaultingError("lifecycle base identity mismatch")
    ...
    if (
        ...
        raise LiveDefaultingError("HCP base identity mismatch")
    ...
    if any((
        ...
        raise LiveDefaultingError("registration base identity mismatch")
    ...
    for name in (
        ...
        if any((
            ...
            raise LiveDefaultingError("Application base identity mismatch")
        if current.get("spec", {}).get("source", {}).get("targetRevision") != amendment["platform"]["sourceCommit"]:
            raise LiveDefaultingError("Application source revision changed")
        replacement = BASE.clean(current)
        ...
        if name.endswith("-core"):
            if current.get("spec", {}).get("ignoreDifferences"):
                raise LiveDefaultingError("Core already has ignoreDifferences")
            if current.get("spec", {}).get("syncPolicy", {}).get("syncOptions") != desired["spec"]["syncPolicy"]["syncOptions"]:
                raise LiveDefaultingError("Core sync options drifted")
            replacement["spec"]["ignoreDifferences"] = copy.deepcopy(desired["spec"]["ignoreDifferences"])
            operation = current.get("status", {}).get("operationState", {})
            if operation.get("phase") != "Failed" or current.get("status", {}).get("health", {}).get("status") != "Healthy":
                raise LiveDefaultingError("Core runtime precondition mismatch")
        elif current.get("spec").get("ignoreDifferences"):
            raise LiveDefaultingError("ignoreDifferences escaped outside Core")
    ...
    if len(prepared) != 13 or prepared[-1][2].split("/")[-1] != "disposable-ok141-observability-core":
        raise LiveDefaultingError("prepared set or Core-last ordering mismatch")

    evidence: dict[str, Any] = {
        "apiVersion": "evidence.openkubes.io/v1alpha1",
        "kind": "OK141LivePlatformDefaultingDiffAmendmentEvidence",
    ...
    try:
        for kubeconfig, category, raw_uri, current, replacement in prepared:
            ...
            if not uid or not resource_version:
                raise LiveDefaultingError("prepared object lacks UID/resourceVersion")
            returned = BASE.raw(
            ...
            if returned.get("metadata", {}).get("uid") != uid or returned.get("metadata", {}).get("resourceVersion") == resource_version:
                raise LiveDefaultingError("optimistic-concurrency postcondition failed")
    ...
    evidence.update({
        "state": "PASS-AMENDED-AUTOMATIC-RECONCILIATION-POSSIBLE",
        "coreExactIgnoreRuleInstalled": True,
...
def main() -> int:
    ...
    try:
        if args.command == "verify":
            ...
            if not args.execute:
                raise LiveDefaultingError("amend requires --execute")

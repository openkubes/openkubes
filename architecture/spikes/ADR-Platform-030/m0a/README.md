# M0a — CAAPH prerequisite protocol

**Ticket:** OK-141

**Baseline:** `main` at `2f6ff0c2e1a9b98847f0652bac31bf144de40ab0`
...
This directory turns the T2a feasibility result into an executable but fully
disabled prerequisite protocol. It does not install CAAPH, submit a
`HelmChartProxy`, create a workload Cluster, or modify the T3 protocol.
...
- `m0a-protocol.yaml` binds the current fixture, installation inventory,
  artifact-resolution chain, blockers, disabled phases, and later acceptance.
- `caaph-installation-inventory.yaml` records the exact upstream release asset
  and expected Kubernetes object inventory.
- `caaph-rbac-review.yaml` records the controller's cluster-wide sensitive
  capabilities and the narrower submitter boundary that still must be rendered
  and proved.
- `submitter-role-candidate.yaml` is the bounded namespace Role candidate. No
...
- `helmchartproxy-v3-candidate.yaml` is current-fixture-bound but explicitly
  ...
  control.
- `verify_m0a_protocol.py` and `tests/test_m0a_protocol.py` enforce the
  fail-closed invariants offline.
- `m0a-protocol.sha256` is informational document identity only. It grants no
  authority.

...
SCRIPT = HERE / "verify_enablement_e1_closure_v1.py"
SPEC = importlib.util.spec_from_file_location("verify_e1", SCRIPT)
...
def test_publication_candidate_binds_exact_redacted_evidence():
    candidate = yaml.safe_load(
        (HERE / "enablement-e1-publication-candidate-v1.yaml").read_text()
    )
...
def test_network_ready_during_fault_must_be_false(tmp_path: Path):
    value = yaml.safe_load(VERIFY.EVIDENCE.read_text())
    value["spec"]["result"]["NetworkReadyDuringFault"] = True

#!/usr/bin/env python3
"""Verify the redacted OK-141 P1 closure checkpoint."""
...
EVIDENCE = HERE / "platform-p1-closure-evidence-v1.yaml"
...
def verify(path: Path = EVIDENCE) -> str:
    ...
    expect(value.get("kind"), "OK141ControlledPlatformFailureClosure", "kind")
    spec = value["spec"]
    ...
    expected_result = {
        ...
        "faultWritten": True,
        "faultObserved": True,
        "failureObservationIteration": 31,
        "PlatformReadyDuringFault": False,
        "coreAndAlertingReadyDuringFault": True,
        "protectedTargetUnchangedDuringFault": True,
        "exactRestoreWritten": True,
        "recoveryObserved": True,
        "recoveryObservationIteration": 1,
    ...
    expected_safety = {
        ...
        "postRunApplicationsReady": True,
        "postRunProtectedTargetUnchanged": True,
    }
    for key, expected in expected_safety.items():
        expect(spec["safety"].get(key), expected, f"safety.{key}")
    expect(spec["ownership"]["desiredPlatformOwner"], "Argo-CD", "owner")

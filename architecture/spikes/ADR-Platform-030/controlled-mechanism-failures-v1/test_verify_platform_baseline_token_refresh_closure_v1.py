...
SCRIPT = HERE / "verify_platform_baseline_token_refresh_closure_v1.py"
SPEC = importlib.util.spec_from_file_location("verify_platform_refresh", SCRIPT)
...
def test_publication_candidate_binds_exact_redacted_evidence():
    candidate = yaml.safe_load(
        (HERE / "platform-baseline-token-refresh-publication-candidate-v1.yaml").read_text()
    )
...
def test_all_applications_must_be_healthy(tmp_path: Path):
    value = yaml.safe_load(VERIFY.EVIDENCE.read_text())
    value["spec"]["result"]["allApplicationsSyncedHealthy"] = False
...
def test_p1_must_not_have_written(tmp_path: Path):
    value = yaml.safe_load(VERIFY.EVIDENCE.read_text())
    value["spec"]["safety"]["P1WritesPerformed"] = True
    path = tmp_path / "tampered.yaml"
    path.write_text(yaml.safe_dump(value))
    with pytest.raises(VERIFY.VerificationError):
        VERIFY.verify(path)

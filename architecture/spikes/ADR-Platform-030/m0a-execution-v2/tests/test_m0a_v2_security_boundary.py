...
from verify_m0a_v2_security_boundary import VerificationError, verify  # noqa: E402


CANDIDATE = ROOT / "m0a-v2-security-boundary.yaml"
DIGEST = ROOT / "m0a-v2-security-boundary.sha256"
...
@pytest.mark.parametrize(
    ...
    [
        (("spec", "state"), "READY"),
        (("spec", "credentialBoundary", "patchAllowed"), True),
        (("spec", "admissionBoundary", "allowedIdentities"), 18),
        (("spec", "revocationBoundary", "immediateRejectionClaim"), True),
        (("spec", "authorization", "retryGranted"), True),
    ],
)
def test_unsafe_boundary_changes_fail_closed(tmp_path: Path, path: tuple[str, ...], value) -> None:

...
| Per-cluster auth: dedicated mount + roles + least-privilege policies + reviewer credential | **central declarative Vault reconciler** (`provider-vault`), driven by an **explicit per-cluster `VaultConfig` XR applied *after* registration** — ADR-013 registration itself creates **no** Vault objects (only the kubeconfig Secret + provider-helm `ProviderConfig`) | see "Vault configuration" + `runbooks/vault-consumer-onboarding.md` |
...
>
> **Root token — revoked (realised ceremony evidence, 2026-07-25).** The initial root token was used only to enable audit, admin auth and the automation seed, then revoked (`vault token revoke -self`, success recorded). **No standing root token exists.** Administrative break-glass is **`userpass/breakglass`** (a strong password under the same Phase-1 custody, delivered over **stdin only** — never argv/history/logs), and recovery uses the Shamir unseal ceremony — not a stored root token. Procedure (Tier A login; Tier B `generate-root` for exactly one action, with audit evidence + mandatory root-revoke proof): `runbooks/vault-breakglass-ceremony.md`.
>
> **Custody verification (mandatory).** Every custody artifact — the unseal-share wrapper and the break-glass password — MUST be verified at creation by a decrypt round-trip / test login, and re-verified on a schedule. An unverified custody artifact is not a backup: a wrong or forgotten passphrase is otherwise discovered only when recovery is already needed. (A lost unseal-share passphrase forced a full re-seed — the safe escape hatch is `vault operator generate-root` with the unseal shares while Vault is still unsealed.)
...
- **Unattended recovery — REALISED (OK-114, 2026-07-27).** Auto-unseal is now the production baseline: ok-shared was migrated Shamir→**Transit** auto-unseal, backed by a dedicated, independently-bootstrapped **Transit Vault on ok-mgmt** (edition-neutral; renewable scoped token; no Vault→Vault recursion — the Transit Vault has its own attended Shamir seal with **decrypt-verified** custody). A full pod restart now recovers **unattended** (verified: all voters returned `Sealed=false` with no manual unseal; consumer Secret unaffected). The former Shamir unseal shares are retained as **recovery keys** (5/3) for admin operations (`generate-root`/rekey). Runbook + verified HA migration choreography: `runbooks/vault-transit-autounseal.md`.
...
**Day-1/Day-2 Vault-config reconciler (decided — item 13):** exactly one authoritative mechanism = Crossplane `provider-vault` on **ok-mgmt**, driven by an **explicit per-cluster `VaultConfig` XR** (one per consuming cluster) that renders the auth mount + policies + roles as Vault managed resources. **Registration (ADR-013) does not itself create these** — it provisions only the kubeconfig Secret + provider-helm `ProviderConfig`; onboarding a consumer to Vault is a separate, explicit apply (`runbooks/vault-consumer-onboarding.md`). Chosen for continuous drift correction, reuse of the running control plane and its ADR-013 `ProviderConfig`, and the absence of a sensitive external state file; a single reconcile loop precludes split-brain over auth config. Terraform (vault provider) was rejected for its drift-only-on-`plan` model and a sensitive state file off the Crossplane/GitOps trajectory; a config Job was rejected as drift-blind/imperative.
...
**Live result (A6 body-scoping probe, 2026-07-29) — this clause now applies.** The reserved-prefix narrowing scopes policy **names** (`okvc-*`) but **not their contents**: the probe confirmed the reconciler identity can author an `okvc-`-named policy carrying a KV **write** body and confer it via an `AuthBackendRole`'s `token_policies`. So "consumer read-only" is a property of the **composition template** (`capabilities=["read"]`), not of the policy boundary. The accurate characterization is therefore **"bounded Vault-config administrator within `okvc-*`"** — not full least-privilege at the content level — and the residual (provider-vault can confer any KV capability inside `okvc-*`) is **Accepted Risk AR-025-2**. Mitigations that make this acceptable for Phase 1: the Composition is the **single** authoring path (the emitted capabilities are template-controlled and three-way-reviewed — see the OK-115 note below), Crossplane authoring is **cluster-admin-gated on ok-mgmt**, and the audit device logs every policy write. **Re-evaluation trigger →** a Vault-side content control (admission controller, or Sentinel on Enterprise) becomes required if a second / less-trusted `provider-vault` consumer appears, if `VaultConfig` authoring is opened beyond platform operators, or on a move to Vault Enterprise. Evidence: `ADR-Platform-025-A6-body-scoping-acceptance-record.md`.

**Claim split (OK-115, 2026-07-30) — the `okvc-*` namespace now legitimately holds a *scoped write* policy.** The composition no longer emits read-only exclusively: a `seedRoles` entry renders **`okvc-<cluster>-<app>-seed`** = `create`/`update` **only** on the derived `secret/data/<cluster>/<app>/*` (no read/list/delete/metadata), bound to **one dedicated ServiceAccount** (never the VSO workload SA — CEL-enforced) with `tokenNoDefaultPolicy: true` and a capped 60–600s TTL; who may mint that SA token is governed by **Kubernetes RBAC** on `serviceaccounts/token`, so there is no standing Vault credential. 18 negative operations were proven denied (`tooling/ok115-scoped-seed-test.sh`). The precise claim is therefore: **workload roles are read-only; seed roles are scoped write-only** — both properties of the reviewed composition template, not of the policy-name boundary. This is the **sanctioned replacement for the break-glass interim seed** (OK-115): it narrows AR-025-2's real-world exposure — routine per-cluster seeding no longer needs an admin identity — without eliminating the residual that the reconciler *could* author arbitrary capability under `okvc-*` (which the re-evaluation trigger above still governs). Evidence: openkubes PR #42; `runbooks/vault-consumer-onboarding.md` §"scoped owner identity".
...
3. **Declarative reconciler** — *decided:* Crossplane `provider-vault` on ok-mgmt, driven by an explicit per-cluster `VaultConfig` XR (registration does **not** create it), K8s-auth automation identity seeded by the bootstrap ceremony (see above). Remaining is *implementation* (the `VaultConfig` XR + provider version pin), not selection.
...
7. **A **fresh** datacenter install receives the Secret from Vault via a `VaultStaticSecret` (VSO) before the observability Helm release** — *verified 2026-07-29* on `ok-obs-verify` (ok-cluster-driven, `REVISION 1`; OpenSearch bootstrapped from the Vault value — Vault KV v2 → HTTP 200, prior file-profile value → 401; contract gate PASS all five). Evidence: `ADR-Platform-025-crit7-fresh-install-acceptance-record.md`. The one situation where the Secret cannot already exist; **not** satisfied by OK-109 Part 1's *file-profile* fresh install (that satisfies OK-109 AC1 + ADR-024, no Vault in the picture). Path: register the cluster (ADR-013), then **apply its `VaultConfig` XR** (registration does **not** create the mount) → provider-vault reconciles the auth mount/policy/role → VSO. Procedure: `runbooks/vault-consumer-onboarding.md`. (Note the OK-115 gap: seeding the KV credential itself has no sanctioned automated route yet — interim seed is a supervised break-glass write.)
...
13. **Day-1/Day-2 Vault-config reconciler** — *selected, narrowed & closed (A6):* Crossplane `provider-vault` **v4.0.1** on ok-mgmt (see §Vault configuration); `VaultConfig` XR implemented, `ok-config-automation` narrowed to reserved prefixes with **deny** on its own seed mount and proven live (negative test passed), ownership migrated to the reserved `okvc-` identity, break-glass rotated and its tokens revoked. **Scope is a *bounded Vault-config administrator within `okvc-*`*, not full least-privilege** — the A6 body-scoping probe (2026-07-29, live) confirmed the narrowing scopes policy names, not contents, so consumer read-only rests on the composition template; the residual is **Accepted Risk AR-025-2** with a content-control re-evaluation trigger (see §Least-privilege scope). **Claim split (OK-115):** workload roles are read-only; **seed roles are scoped write-only** (`okvc-<cluster>-<app>-seed` = create/update on the derived path only, dedicated SA, `tokenNoDefaultPolicy`, capped TTL, k8s-RBAC-gated) — the sanctioned replacement for the break-glass interim seed, 18 negative ops proven denied. Evidence: `ADR-Platform-025-A6-ownership-migration-acceptance-record.md` + `ADR-Platform-025-A6-body-scoping-acceptance-record.md` + openkubes PR #42.

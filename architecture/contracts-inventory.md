...
| OS Capability Contract | ADR-Platform-016 | `ok-linux` (Talos / Flatcar) | ✅ v0.1.1 |
| Persistent Storage Contract | ADR-Platform-009 | `ok-storage` (Longhorn, three storage classes) | ✅ v0.1.0 |
| Ingress Contract | ADR-Platform-010 | Traefik / Cilium, ingress class `ok-ingress` | ✅ |
| Observability Capability Contract | ADR-Platform-018, ADR-Platform-024, ADR-Platform-027 | `ok-observability` | 🚧 scaffold |
| GitOps Contract | ADR-Platform-011 | `ok-gitops` (ArgoCD) | 📋 planned |
| Secret Contract | ADR-Platform-011, ADR-Platform-025 | open — Vault, Infisical, or cloud secrets manager (the contract, not the product, is the capability) | 📋 planned |
...
| Artifact Registry Capability Contract | ADR-Platform-028 | `ok-registry` (GHCR / Harbor / Local Registry) | 🚧 scaffold |
| Messaging Capability Contract | ADR-Platform-029 | `ok-messaging` (NATS / RabbitMQ) | 📋 planned |
| DBaaS Capability Contract | ADR-Platform-032 | `ok-dbaas` (PostgreSQL / OpenSearch operators) | 📋 planned |
| Robotics Fleet Orchestration Contract | ADR-Platform-019 | `ok-robotics` | 🚧 scaffold |
...
- **Edge Node Profile Contract** — constrained profile interfaces introduced in ADR-Platform-014.